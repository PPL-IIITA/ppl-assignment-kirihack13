...
void breakUp(vector<rel> &r, vector<boy> &b, int kin, int t){
	...
	while(z < k && input.eof() == 0){
		...
		input >> name >> y >> attractiveness >> y >> intelligence >> y >> min_attract >> y >> x >> y >> type >> y >> budget;//!info is taken input from blist
		while(iterator != r.end()){
			if(name == iterator->getBname()){
				if(iterator->getHappiness() >= t && ls > kin){ //!condition to check required conditions of Q4 and Q6
					m = true; 
				}else{
					m = false;
				} 
	...
	input.close();
	input.open("couple_full.csv"); //!retrieving couple info from saved file
	string x;
	int ref = 1;
	if(kin != 0){
		ref = 0; //!check so that in case of Q6 where kin is passed 0 loop shouldnt run infinite
	}
	while(input.eof() == 0){
		ls = 1;
		iterator = r.begin();
		...
		input >> gname >> x >> bname >> x >> rem_maintenance >> x >> budget >> x >> gintelligence >> x >> bintelligence >> x >> gattractiveness >> x >> battractiveness >> x >> gtype >> x >> pref >> x >> btype;//!info taken input from couple full info file
			...
			if(iterator->getGname() == gname){
				...
				for(iterator1 = b.begin(); iterator1 < b.end(); iterator1++){ //!finding suitable boy for girl according to her preference
					if(iterator1->getBudget() > budget && iterator1->getIs_commited() == false){
						if(pref == 1 && iterator1->getBudget() > max_budget){ //! calculating boyfriend with max budget
							max_budget = iterator1->getBudget();
							i = 1;
							iterator2 = iterator1;
						}
						else if(pref == 2 && iterator1->getAttractiveness() > max_att){ //!calculating boyfriend with max attractiveness
							max_att = iterator1->getAttractiveness();
							i = 1;
							iterator2 = iterator1;
						}
						else if(pref == 3 && iterator1->getIntelligence() > max_intl){ //!calculating boyfriend with max intelligence
